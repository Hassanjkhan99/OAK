...
namespace OAK
{
    ...
    public class LexicalAnalyzer
    {
        ...
        private Token IsChar(string text, int lineNumber)
        {
            Token token = null;
            ...
            //         // {
            //         //     //don't include the new l_num character in the value part
            //         //     token = new Token(ClassPart.CHAR_CONSTANT, text.Substring(0, i), lineNumber);
            //         //     break;
            //         // }

            //         if (text[2] == '\'' && Regex.Match(text[1].ToString(),"[A-Za-z]").Success)
            //         {

            //             token = new Token(ClassPart.CHAR_CONSTANT, text.Substring(0, 2 ), lineNumber);
            ...
            //         //     return new Token(ClassPart.CHAR_CONSTANT, text.Substring(0, i + 1), lineNumber);
            //         // }
            //         // //if text ends and we don't find the ending quote for string, means string is not closed
            //         // if (i == text.Length - 1)
            //         //     token = new Token(ClassPart.INVALID, text, lineNumber);
            ...
            if (text.StartsWith('\'') && text.Length == 3)
            {
                if (Regex.Match(text[1].ToString(), "[a-z]").Success)
                {
                    return new Token(ClassPart.CHAR_CONSTANT, text.Substring(0, 3), lineNumber);
                }
            }
            if(text.StartsWith("\'\\") && text.Length == 4){
                                if (Regex.Match(text[2].ToString(), "[a-z]").Success)
                {
                    return new Token(ClassPart.CHAR_CONSTANT, text.Substring(0, 4), lineNumber);
                }
            }
